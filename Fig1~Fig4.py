...
data_array.to_netcdf('Your path/TPwind.nc')                 
                


data_array = xr.DataArray(chucun, dims=['lat', 'lon'], coords={'lat': lat, 'lon': lon}, name='TPwind')
ds = xr.Dataset({'TPwind': data_array})   
data_array.to_netcdf('Your path/TPwind_tested.nc')  


data_array = xr.DataArray(chucunqian, dims=['lat', 'lon'], coords={'lat': lat, 'lon': lon}, name='TPwind')
ds = xr.Dataset({'TPwindbefore': data_array})   
data_array.to_netcdf('Your path/TPwindbefore.nc')  

data_array = xr.DataArray(chucunhou, dims=['lat', 'lon'], coords={'lat': lat, 'lon': lon}, name='TPwind')
ds = xr.Dataset({'TPwindbefore': data_array})   
data_array.to_netcdf('Your path/TPwindafter.nc') 

import matplotlib as mpl
from matplotlib.colors import LinearSegmentedColormap
def china_map_feature():
    provinces=cfeature.ShapelyFeature(shpreader.Reader('wind-change-on-plant-growth/eleven/2022省矢量.shp').geometries(),
                                      ccrs.PlateCarree(),edgecolor='k',facecolor='none')
    nine_lines=cfeature.ShapelyFeature(shpreader.Reader('wind-change-on-plant-growth/eleven/九段线.shp').geometries(),
                                      ccrs.PlateCarree(),edgecolor='k',facecolor='none')
...
shps=shpreader.Reader('wind-change-on-plant-growth/eleven/国家矢量.shp')
...
f1 = xr.open_dataset('Your path/TPwindbefore.nc')
lat=f1.lat
lon=f1.lon
...
# In[]   Fig.2c
f1 = xr.open_dataset('Your path/TPwindafter.nc')
from matplotlib.colors import TwoSlopeNorm
...
shps=shpreader.Reader('wind-change-on-plant-growth/eleven/eleven/国家矢量.shp')
f2_ax1.add_geometries(shps.geometries(),ccrs.PlateCarree(),
                  linewidth=0.5,edgecolor='k',facecolor='none')
provinces,nine_lines=china_map_feature()
f2_ax1.add_feature(cfeature.OCEAN,color='w',zorder=0)
...
from matplotlib.colors import TwoSlopeNorm
 ...
 #'G:/CN05.1/month/0.25_nearest_rec_mon.nc', This one should run LAIdataprocess.py to generate firstv
f=xr.open_dataset('G:/CN05.1/month/0.25_nearest_rec_mon.nc') 

f1 = xr.open_dataset('Your path/TPwind.nc')
